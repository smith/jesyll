...
var DefaultConverters = function(templates) {
  return {
  ...
  return {
    run: function() {
      ...
      var templatesDir = source.join(options.get('templates-dir')),
          ...
          converters = options.converters || DefaultConverters(templates);
      ...
      for (var i = 0; i<sourceFiles.length; i++) {
        ...
        if (sourceFile.charAt(0) == '_' || ext.match(sourceExtRegex)) {
        ...
        print("extracted " + json.stringify(extracted));
        ...
        // Then put the body into its template
        try {
          var html = templates.makeHtml(metadata, body);
        } catch (e) {
        ...
        var newName = sourceFile.slice(0, -ext.length) + '.html';
