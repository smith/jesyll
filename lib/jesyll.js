...
exports.jesyllApp = jesyllApp = function(source, dest, options) {
  return {
    'run': function() {
      ...
      var layout = layoutsDir.join('default.html').open().read();
      var template = jsontemplate.Template(layout, {meta: "{{}}"});
      ...
      for (var i=0; i<sourceFiles.length; i++) {
        var filename = postsDir.join(sourceFiles[i]);
