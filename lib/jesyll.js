    ...
    makeBody: function(filename, sourceType, content) {
      ...
      switch (sourceType) {
        ...
        case 'json':
          ...
          var defaultTemplate = filename.slice(0, -ext.length) + '.jsont',
              templateName = jsonData.$template || defaultTemplate,
              t = file.path(templateName).open().read(),
  ...
  return {
    run: function() {
      ...
      var converters = options.converters || DefaultConverters(),
          templatesDir = source.join(options.get('templates-dir')),
