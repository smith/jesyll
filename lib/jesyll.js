...
var handleSource = function(sourceTree, sourceFile) {
  ...
  return util.ObjComposite(extracted, parsed);
}
  ...
  return {
    run: function() {
      ...
      for (var i = 0; i<sourceFiles.length; i++) {
        ...
        var docData = handleSource(sourceTree, sourceFile),
            expanded = util.expandSpecial(docData, globals, sourceTree),
            doc = util.Composite(globals, defaultDoc, expanded);
