...
var FileHandler = function(
  ...
  that.stopIndex = function() {
    log.info('Building indices for %s', that.destDir);
    ...
    for (var i = 0; i < indexDocs.length; i++) {
      ...
      docStack.push({
          body: body,
          // HACK
          dest: {dir: that.destDir, basename: 'index', ext: '.html'},
  ...
  that.enterDir = function(dirName) {
    ...
    that.indexStack[that.indexStack.length-1]['dirs'].push(
        {name: that.destDir});
  ...
  that.onFile = function(filename) {
    ...
    if (isSource) {
      ...
      if (docStack.get('.index')) {
        that.indexStack[that.indexStack.length-1]['index-docs'].push(
            relativePath)
        skip = true;
      }
