...
var Templates = function(templatesDir) {
  ...
  that.makeHtml = function(doc) {
    ...
    return template.expand(doc);
  };

  return that;
}

// $ and / is necessary for special front matter (see expandSpecial)
var META_RE = /^\s*(\S+):\s*(.*)/;
...
var handleSource = function(sourceTree, sourceFile, globals) {
  ...
  if (ext == '.json') {
    // Allow people to name things foo.py.json?
    extracted = extractMetadata(sourceFile.slice(0, -ext.length));
    content = f.read();
  ...
  if (parsed['.hidden']) {
    ...
    return util.ObjComposite(parsed);
  } else {
    var expanded = util.expandSpecial(parsed, globals, sourceTree);
    return util.ObjComposite(extracted, expanded);
  }
}
  ...
  return {
    run: function() {
      ...
      var sourceTree = FileSystem(options.get('source-dir')),
          ...
          // use .get() on globals
          globals = util.ObjComposite(options.get('globals'));
      ...
      for (var i = 0; i<sourceFiles.length; i++) {
        ...
        // not variables.  doc.get('template')
        var defaultDoc = util.ObjComposite({
            '.template': 'default',
        ...
        var docData = handleSource(sourceTree, sourceFile, globals);

        // TODO: unhide flag
        if (docData.get('.hidden')) {
          printf('Skipping hidden file %s', sourceFile);
          continue;
        }

        var doc = util.Composite(globals, defaultDoc, docData);

        print("title: " + doc.get('title'));

        if (doc.get('body-type') !== 'html') {

          // Convert the body first
          try {
            var body = conv.makeBody(doc);
            // Overlay the new body on the old one
            var doc = util.Composite(doc, util.ObjComposite({body: body}));
          } catch (e) {
