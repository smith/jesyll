...
var FileHandler = function(
    docStack, pluginStack, sourceTree, buildDoc, copy, options) {
  ...
  that.enterDir = function(dirName) {
    ...
    var currentDir = that.dirStack.join('/'),
        plugins = readPlugins(sourceTree, currentDir);
    that.pluginStack.push(plugins);

    var dirConfig = readConfig(sourceTree, currentDir),
        vars = dirConfig.vars || {};
    docStack.push(vars);
  };
...
exports.jesyllApp = jesyllApp = function(options) {
  ...
  var pluginStack = [readPlugins(sourceTree)];
