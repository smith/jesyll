...
// Global variable that __plugins.js should augment.
var PLUGINS = {};
...
var FileHandler = function(
  ...
  that.enterDir = function(dirName) {
    ...
    var pluginsEntry = {plugins: that.readPlugins(), pushedToDocStack: false};
    that.pluginStack.push(pluginsEntry);

    var dirConfig = readConfig(sourceTree, that.dirStack.join('/'));
    docStack.push(dirConfig);
    
    if (pluginsEntry.plugins.onPush) {
      log.info('Running onPush');
      try {
        var result = pluginsEntry.plugins.onPush(docStack);
      } catch (e) {
        log.error('Caught exception executing onPush');
        throw e;
      }
      if (result) {
        docStack.push(result);
        pluginsEntry.pushedToDocStack = true;
      }
    }
  };
  ...
  that.exitDir = function() {
    var pluginsEntry = that.pluginStack.pop();
    if (pluginsEntry.pushedToDocStack) {
      docStack.pop();
    }
    docStack.pop();
    that.dirStack.pop();
  ...
  that.readPlugins = function() {
    var dir = that.dirStack.join('/'),
        path = file.join(dir, '__plugins.js');
    try {
      var contents = sourceTree.contentsOf(path);
    } catch (e) {
