...
var handleSource = function(sourceTree, sourceFile) {
  ...
  var defaults = {
      ...
      'relative-path': sourceFile,
      filename: file.basename(sourceFile),

      // TODO: Should this be on top?  Otherwise content: in the front matter
      // can override it
      content: content
      };
  var extracted = extractMetadata(sourceFile),
      parsed = parseFrontMatter(metadataLines),
      extractedAndParsed = util.Composite(defaults, extracted, parsed),
      expanded = util.expandSpecial(parsed, extractedAndParsed, sourceTree);
      doc = util.ObjComposite(defaults, extracted, expanded);
