...
var FileHandler = function(
    docStack, options, sourceReader, docBuilder, copier) {
  ...
  that.onFile = function(filename) {
    ...
    if (isSource) {
      ...
      for (var i = 0; i < items.length; i++) {
        log.info('Before pushing: %s', docStack.toString());
        log.info('pushing %s', items[i]);
      ...
      if (!hidden) {
        var error = docBuilder.build(docStack);
        if (error) {
      ...
      for (var i = 0; i < items.length; i++) {
        docStack.pop();
        log.info('After popping: %s', docStack.toString());
      }
      log.pop();
...
function FileCopier(sourceTree, destTree, dirMaker) {
  return {
...
//   destTree: destination tree
function DocBuilder(conv, templates, destTree) {
  return {
...
exports.jesyllApp = jesyllApp = function(options) {
  ...
  var sourceReader = SourceReader(sourceTree),
      ...
      docBuilder = DocBuilder(conv, templates, destTree, dirMaker),
      copier = FileCopier(sourceTree, destTree, dirMaker);
