...
var VarStack = exports.VarStack = function() {
  ...
  that.get = function(name) {
    ...
    for (var i = objs.length-1; i >= 0; i--) {
      ...
      if (value === undefined) {
        continue;
      } else if (isArray(value)) {  // A sequence of operations to carry out
        // TODO: apply ops
        ...
        for (var j = 1; j < value.length; j++) {
          switch (value[j]) {
            ...
            case '$':
              //log.info("Expanding template '%s'", current);
              var t = jsontemplate.fromString(current);
              // RECURSIVE construction of VarStack
              current = t.expand(VarStack(objs));
              //log.info("Expanded to '%s'", current);
              break;

            case '&':
              log.debug('READING %s', filename);
              if (fs) {
                return fs.contentsOf(filename);
              } else {
                throw {
                  name: 'NoFileSystem',
        ...
        //log.info("Value of %s is '%s'", name, current);
        return current;

      } else if (value !== undefined) {  // It's a normal object; just return it
        return value;
      }
    }
