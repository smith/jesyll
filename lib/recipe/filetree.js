...
// Part of "oil"
//
// Runtime component for a tree embedded by Gelatin
//
// Args:
//   dataFunc: A function that returns the embedded data
var GelatinEmbeddedTree = exports.GelatinEmbeddedTree = function(dataFunc) {
  ...
  that.contentsOf = function(path, errorVal) {
    // Lazily evaluate the data, since it may be big
    if (data === null) {
      data = dataFunc();
    }
    ...
    if (lines === undefined) {
      if (errorVal === undefined) {
        throw {
           name: 'FileNotFound',
           message: path + ' not found in EmbeddedTree' };
      }
      return errorVal;
    }
    cache[path] = lines.join('');
    return cache[path];
  }
