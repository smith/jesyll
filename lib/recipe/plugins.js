...
PluginManager.prototype = {
  ...
  empty: function() {
    return this.pluginStack.length === 0;
  },

  // Mutates the docStack
  onDoc: function(docStack) {
    // Now push all __plugins.js functions
    var numPushed = 0;
    for (var i = 0; i < this.pluginStack.length; i++) {
      var entry = this.pluginStack[i];
      if (entry.onDocPush) {
  ...
  enterDir: function(dirName) {
    this.pluginStack.push(readPlugins(this.sourceTree, dirName));
  },
