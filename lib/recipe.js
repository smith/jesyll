...
var readConfig = function(sourceTree, dir) {
  var path;
  if (dir) {
    path = oil.join(dir, '__config.json');
  } else {
    path = '__config.json';
  }
  try {
...
exports.recipeApp = recipeApp = function(options) {
  var sourceTree = oil.FileSystem(options.get('source-tree')),
      destTree = oil.FileSystem(options.get('dest-tree'));
...
var recipeMain = function (args) {
    ...
    var config = readConfig(oil.FileSystem(sourceDir));
